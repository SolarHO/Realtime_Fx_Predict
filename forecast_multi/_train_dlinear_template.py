...
def save_pg(dates, y_pred, last_true):
    ...
    from sqlalchemy import create_engine, MetaData, Table, Column, Date, String, Float
    from sqlalchemy.dialects.postgresql import insert
    import math

    engine = get_engine()  # 기존 코드에 있는 함수 사용
    pair = globals().get('PAIR', 'USDKRW')
    model = 'dlinear'
    ...
    for d, y in zip(dates, y_pred):
        rows.append({
            ...
            'pair': pair,
            'y_true': float(last_true) if last_true is not None and not math.isnan(last_true) else None,
